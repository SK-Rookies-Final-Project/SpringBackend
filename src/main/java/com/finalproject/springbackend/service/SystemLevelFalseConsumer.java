...
public class SystemLevelFalseConsumer {
    ...
    public void startConsumerForUser(String username, String password) {
        if (userConsumers.containsKey(username)) {
            log.info("사용자 {}의 SystemLevelFalseConsumer 이미 실행 중", username);
            return;
        }
        
        log.info("사용자 {}의 SystemLevelFalseConsumer 시작", username);
                ...
                try {
                    ...
                    while (!Thread.currentThread().isInterrupted()) {
                        ...
                        for (ConsumerRecord<String, String> record : records) {
                            ...
                            log.info("시스템 권한 부족 로그 수신: {}", message);
    ...
    public void stopConsumerForUser(String username) {
        ...
        if (consumer != null) {
            log.info("사용자 {}의 SystemLevelFalseConsumer 중지", username);
            consumer.close();
        }
    ...
    private void sendMessageToClients(String message) {
        // 기존 방식 (하위 호환성)
        Map<String, SseEmitter> emitters = sseService.getSystemLevelFalseEmitters();
        ...
        // 사용자별 SSE 연결에도 전송
        Map<String, Map<String, SseEmitter>> allUserEmitters = sseService.getAllUserSystemLevelFalseEmitters();
